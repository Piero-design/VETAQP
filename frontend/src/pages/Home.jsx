...
export default function Home() {
  ...
  const filtered = q
    ? items.filter((x) => (x.name + x.description).toLowerCase().includes(q.toLowerCase()))
    : items;
  ...
  return (
    <div className="space-y-6">
      <div className="card p-6 bg-gradient-to-r from-brand/10 to-mint/10">
        ...
        <p className="text-gray-600">Productos y servicios seleccionados por veterinarios.</p>
        <div className="mt-4">
          <input
            className="input"
        ...
        {filtered.map((p) => (
          <div key={p.id} className="card p-4">
            <img className="w-full h-40 object-cover rounded-xl mb-3"
                 src={p.image || "https://placehold.co/600x400"} alt={p.name}/>
            <h3 className="font-semibold">{p.name}</h3>
            <p className="text-sm text-gray-500">{p.description}</p>
            <div className="mt-3 flex items-center justify-between">
              <span className="font-semibold">S/ {Number(p.price).toFixed(2)}</span>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {["Consulta médica", "Baño y corte", "Vacunación"].map((s, i) => (
          <div key={i} className="card p-5">
            ...
            <p className="text-sm text-gray-500">Agenda tu cita con nuestros especialistas.</p>
